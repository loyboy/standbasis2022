...
class AuthController extends Controller
{
    ...
    public function sendVerificationCodeToMobile(Request $request)
    {          
        try {
            $user = null;
            ...
            if (!$user) {
                ...
                $smsData =  $this->sendSMSWithout243( $mobile, $token );
                $securedToken = Hash::make($token);
                $response = PhoneVerification::create([
                    'mobile' =>  $mobile,
                    'pin' =>  $securedToken,
                    'sms_response' =>   $smsData,
                    'sms_request' =>   json_encode(["mobile"=>$mobile , "token"=> $token])
                ]);            
                return $this->successResponseV3(["token"=> $response], "Please enter the verification code sent to $mobile", 200);
            }else{
                return $this->successResponseV3(["user"=> $user], 'Account exists. please update', 400);
            }
        } catch (\Throwable $error) {
            return  $error;
            return $this->errorResponse($error, 'Login failed', 422);
    ...
    public function validateVerificationPin(Request $request)
    {          
        try {
            $user = null;
            $validator = Validator::make(
            ...
            $mobile = $this->remove234FromMobile($request->get('mobile'));
            $response = PhoneVerification::where([
                'id'=>  $request->get('id'), 
            ])->first();
            $response->mobile = $mobile;
            if(Hash::check($request->get('pin'), $response->pin)){
                $user = User::where(['mobile'=> $mobile])->without('cat_list')->without('store_list')->first();  
                return $this->successResponseV3(["token"=>$response, "user"=>$user], "Mobile number verified sucessfully", 200);
            } else{
                return $this->errorResponse(null, 'The pin you entered is invalid', 400);
            }           
        } catch (\Throwable $error) {
            return  $error;
            return $this->errorResponse($error, 'Login failed', 422);
    ...
    public function loginwithpin(Request $request)
    {
        try {
            ...
            if (Hash::check($request->pin, $user->trans_pin)) {
                $tokenResult = $user->createToken('plenti');
                $token = $tokenResult->token;
                return $this->token($tokenResult, 'Login successful', 200, $user);
                //return $this->successResponseV3(['access_token' => $tokenResult->accessToken, 'token_type' => 'Bearer', 'user' => $user], "Login successful");
            } else {
                return $this->errorResponse(null, 'Credentials mismatch', 400);
            }
    ...
    public function onBoardExistingMemebers(Request $request)
    {
        try {
            ...
            $user = User::where('mobile', $request->mobile)->with("transStat")->with("card")->first();
            ...
            if ($user->save()) {
                ...
                return $this->token($this->getPersonalAccessToken(), 'Your profile updated successfully', 200);
            }           
    ...
    {
        $current_user = User::where('id', Auth::id())->with("transStat")->with("card")->with("merchant")->first();
        if ($current_user) {
    ...
    public function getUserById(Request $request)
    {
        ...
        $current_user = Customer::where('id',$request->id)->with("transStat")->with("card")->with("transactionsLog")->with('status')->first();
        if ($current_user) {
            return $this->success('record found', $current_user);
        } else {
            return $this->success('user not found');
    ...
    public function getAllUser(Request $request){
        if($request->input('search') != ''){        
            if(strlen($request->input('search')) > 7)
    ...
    public function getUsersData($request)
    {
        ...
        if ( $request->input('client') ) {
    	    return User::select('id', 'first_name', 'last_name','username','email','mobile','role', 'verified', 'state')->get();
    	}

        $columns = ['id', 'first_name', 'last_name','username','email','mobile','role', 'verified', 'state', 'city'];
        ...
        $query = User::select('id', 'first_name', 'last_name','username','email','mobile','role', 'verified','state', 'city')->orderBy($columns[$column], $dir);
        ...
        $stats = [
            "total_users"=>  Customer::where(['role'=>$role])->count(),
            "verified_users"=>  Customer::where(['status_id'=>$this->Active_STATUS, 'verified'=>$this->VERIFIED ,'role'=>$role])->count(),
            "unverified_users"=>  Customer::where(['status_id'=>$this->PENDING_STATUS, 'verified'=>$this->UNVERIFIED ,'role'=>$role])->count(),
            "blocked_users"=>  Customer::where(['status_id'=>$this->BLOCKED_STATUS,'role'=>$role])->count(),
        ];
    ...
    public function getMerchantData(Request $request )
    {
        ...
        $columns = ['id',  
        'store_name','email', 'mobile','daalu_commision',
        'daalu_profit','customers_earnings'];
        ...
        $query = Store::select('store_name',  'email', 'mobile','daalu_commision','daalu_profit','customers_earnings')
        ...
        $stats = [
            "total_stores"=>  Store::where(['merchant_id' => $merchatId ])->count(),
            "total_deals"=>  Deal::where(['status'=>$this->Active_STATUS, 'merchant_id' => $merchatId ])->count(),
            "total_transactions"=>  CustomerTransactionsLog::where([ 'merchant_id' => $merchatId ])->count(),
        ];
    ...
    public function createPin(Request $request)
    {
        try {
            ...
            $user->save();
            return $this->success('Account updated successfully', (object) []);
        } catch (\Throwable $th) {
            return $this->error($th->getMessage(), 422);
        }
    }
